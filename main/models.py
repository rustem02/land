...
class Questionaire(models.Model):
    ...
    company_ceo_has_tech = models.BooleanField(choices=choices,  verbose_name='Имеет ли управляющий (директор) техническое образование?')
    company_has_filial = models.BooleanField(choices=choices,verbose_name='Есть ли у Вашего предприятия филиалы в других регионах РФ? ')
    ...
    prom_kipia = models.BooleanField(choices=choices,verbose_name='Промышленная автоматика и КИПиА, в т.ч.')
    non_contact = models.BooleanField(choices=choices,verbose_name='Бесконтактные датчики')
    optical = models.BooleanField(choices=choices,verbose_name='Оптические датчики')
    indikator = models.BooleanField(choices=choices,verbose_name='Управление и индикация')
    temperature = models.BooleanField(choices=choices,verbose_name='Датчики давления и температуры')
    ibp = models.BooleanField(choices=choices,verbose_name='Блоки питания')
    encoder = models.BooleanField(choices=choices,verbose_name='Энкодеры')
    converter = models.BooleanField(choices=choices,verbose_name='Частотные преобразователи')
    motor_reductor = models.BooleanField(choices=choices,verbose_name='Мотор-редукторы и электродвигатели')
    baumer = models.BooleanField(choices=choices,
        verbose_name='Датчики Baumer (Швейцария/Германия): оптические, лазерные, ультразвуковые, касания, индуктивные, емкостные')
    process_baumer = models.BooleanField(choices=choices,
    ...
    cement_pesok_zerno = models.BooleanField(choices=choices,
        verbose_name='Датчики уровня сыпучих материалов (песок, цемент, зерно, комбикорм)')
    convair_security = models.BooleanField(choices=choices,
    ...
    light_indication = models.BooleanField(choices=choices,
        verbose_name='Светосигнальное оборудование (светосигнальные колонны и маячки)')
    other_device = models.BooleanField(choices=choices,verbose_name='Другое')
